...
Environment:
  ...
  CLAUDE_SKILLS_DIR  Claude skills directory. Defaults to \${CLAUDE_HOME:-\$HOME/.claude}/skills
USAGE
}
...
if [[ "${INSTALL_CODEX}" -eq 1 ]]; then
  sync_to "${CODEX_SKILLS_DIR}"
fi

if [[ "${INSTALL_CLAUDE}" -eq 1 ]]; then
  sync_to "${CLAUDE_SKILLS_DIR}"
fi
